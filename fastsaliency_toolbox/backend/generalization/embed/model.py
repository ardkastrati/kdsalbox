...
class Decoder(nn.Module):
    def __init__(self, has_bias):
        ...
        self._conv7_3 = nn.Conv2d(1280, 64, kernel_size=3, padding=1)
    ...
    def forward(self, xb, weights):
        ...
        xb = F.relu(F.conv2d(xb, w_weights[0], b_weights[0]))
        xb = F.interpolate(xb, scale_factor=2)
        xb = F.relu(self.bn10_2(self.conv10_2(xb)))
        xb = F.interpolate(xb, size=(480, 640))

        # xb = F.relu(self.bn7_3(xb))
        # xb = self.upsample(xb)
    ...
    def get_param_shapes(self):
        param_shapes = [] # list of lists containing the shapes of the weights
...
class Student(nn.Module, MainNetInterface):
    def __init__(self):
        ...
        self._external_params.extend(list(self.decoder._conv7_3.parameters()))
